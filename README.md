...
A script to build and run the Cylinder flow validation is provided in the scripts directory.

A CPU_target and GPU_target ar outputted in the bin folder. Boundary conditions are channel flow by default and GPU code is generated by default. To change these options, pass a compiler option while building with CMake:

`cmake -DPERIODIC=ON -DUSE_GPU=OFF ..`
