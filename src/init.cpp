...
void initialise_lid(int Nx, int Ny, int Q, float u_lid, float* f, float* rho_arr, float* ux_arr, float* uy_arr)
{
	...
	for (int y = 0; y < Ny; y++)
		for (int x = 0; x < Nx; x++)
		{
            ...
            f[f_idx_cpu(Nx, Ny, x, y, 0)] = w_rho0*(c                           );
            f[f_idx_cpu(Nx, Ny, x, y, 1)] = w_rho1*(c + 3.*ux  + c2*uxsq        );
            f[f_idx_cpu(Nx, Ny, x, y, 2)] = w_rho1*(c + 3.*uy  + c2*uysq        );
            f[f_idx_cpu(Nx, Ny, x, y, 3)] = w_rho1*(c - 3.*ux  + c2*uxsq        );
            f[f_idx_cpu(Nx, Ny, x, y, 4)] = w_rho1*(c - 3.*uy  + c2*uysq        );
            f[f_idx_cpu(Nx, Ny, x, y, 5)] = w_rho2*(c + 3.*uxuy5 + c2*uxuy5*uxuy5);
            f[f_idx_cpu(Nx, Ny, x, y, 6)] = w_rho2*(c + 3.*uxuy6 + c2*uxuy6*uxuy6);
            f[f_idx_cpu(Nx, Ny, x, y, 7)] = w_rho2*(c + 3.*uxuy7 + c2*uxuy7*uxuy7);
            f[f_idx_cpu(Nx, Ny, x, y, 8)] = w_rho2*(c + 3.*uxuy8 + c2*uxuy8*uxuy8);
	    }
}